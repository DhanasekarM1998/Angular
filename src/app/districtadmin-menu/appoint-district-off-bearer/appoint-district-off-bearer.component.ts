...
export class AppointDistrictOffBearerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.fulluserDetails = JSON.parse(
      localStorage.getItem('login_user_details')
    );
    console.log(this.fulluserDetails);
    var today = new Date();
    var minAge = 18;
  ...
  postdata(
    ...
    if (
      ...
      if (this.officebearerform.get('mode').value === '1') {
        console.log('da');
        this.formData.append('user_id', this.fulluserDetails.id);
  ...
  onSelectChange(event: any) {
    // This function will be called when the select element's value changes
    // console.log(this.fulluserDetails);

    const user_party_designation = this.fulluserDetails.party_designation;
    const isSelectedValueIncluded1 = user_party_designation
      .split('/')[1]
      ?.trim();
    console.log(isSelectedValueIncluded1);
    // console.log(user_party_designation);
    const selectedValue = event.target.value;

    console.log('Selected value:', selectedValue);
    // Check if selectedValue is included in user_party_designation
    // const isSelectedValueIncluded = user_party_designation.includes(selectedValue);
    // console.log(isSelectedValueIncluded)
    if (
      (isSelectedValueIncluded1 === 'மாவட்ட அமைப்பாளர்' &&
        selectedValue == 'மாவட்ட அமைப்பாளர்') ||
      (isSelectedValueIncluded1 === 'மாநகர அமைப்பாளர்' &&
        selectedValue == 'மாநகர அமைப்பாளர்')
    ) {
      this.partyDesignation = selectedValue;

     
      if(this.fulluserDetails.image){
        this.ApiService.getImage(this.fulluserDetails.id).subscribe((data) => {
          //console.log(data);
          this.spinner = false;
          let objectURL = 'data:image/jpeg;base64,' + data.image;
      ...
      if(this.fulluserDetails.email){
        this.test_email=='false'
      }
      else{
        this.test_email=='true'
      }
      if(this.fulluserDetails.whatsapp_no){
        this.test_ph =='false'
      }
      else{
        this.test_ph =='true'
      ...
      this.officebearerform.patchValue({
        firstname: this.fulluserDetails.firstname,
        lastname: this.fulluserDetails.lastname,
        father_name: this.fulluserDetails.lastname,
        contact_no: this.fulluserDetails.contact_no,
        whatsapp_no: this.fulluserDetails.whatsapp_no,
        approval_status: this.fulluserDetails.approval_status,
        location_id: this.fulluserDetails.location_id,
        mode: this.fulluserDetails.mode,
        email: this.fulluserDetails.email,
        date_of_birth: this.fulluserDetails.date_of_birth,
        mother_name: this.fulluserDetails.mother_name,
        educational_qualification:
          this.fulluserDetails.educational_qualification,
        degree_major: this.fulluserDetails.degree_major,
        additional_qualification: this.fulluserDetails.additional_qualification,
        fb_id: this.fulluserDetails.fb_id,
        twitter_id: this.fulluserDetails.twitter_id,
        profession: this.fulluserDetails.profession,
        flat_no: this.fulluserDetails.flat_no,
        street_name: this.fulluserDetails.address1,
        town_city: this.fulluserDetails.town_city,
        taluk: this.fulluserDetails.taluk,
        pincode: this.fulluserDetails.pincode,
        party_comments: this.fulluserDetails.party_comments,
